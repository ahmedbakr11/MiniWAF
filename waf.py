...
def allow_by_rate(ip: str) -> bool:
    ...
    while dq and now - dq[0] > WINDOW:          # Remove old timestamps
        dq.popleft()
    if len(dq) >= LIMIT:
        return False
    dq.append(now)
...
async def inspect_request(req: Request, body_text: str) -> Dict[str, Any]:
    ...
    # Raw values
    raw_url = str(req.url)           # full URL
    raw_path = req.scope.get("path", req.url.path)
    raw_query = req.url.query

    # Decode encoded characters
    path = unquote(raw_path)
    query_str = unquote(raw_query)
    decoded_url = unquote(raw_url)

    # --- Quick traversal check (raw URL inspection) ---
    if "../" in decoded_url or "%2e%2e" in decoded_url.lower():
        return {"block": True, "reason": "path_traversal", "rule_id": "TRAV-RAW"}
    ...
    # --- Rate limit ---
    if not allow_by_rate(client_ip):
